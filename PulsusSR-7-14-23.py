...
def Patterning(a):
    ...
    holdStack = [[x for x in holdStack[0] if x[1] > (notes[a][1] + 10)],
                 [x for x in holdStack[1] if x[1] > (notes[a][1] + 10)]]
    holdStackApp = [[x for x in holdStack[0] if x[0] < (notes[a][1] - 10)],
                    [x for x in holdStack[1] if x[0] < (notes[a][1] - 10)]] #Applicable holdStack, takes out notes too close to the starting point of the hold.
    ...
    if holdInd.index(holdInd[a]) != a: #Uses the correct finger if it's a release.
        ...
        patternBonus = rhythmBonus * holdBonus
    else:
        for b in range(4): #Beginning of individual finger checks
            currentStrains[b] = [finStrain[0][1], finStrain[0][1], finStrain[1][0], finStrain[1][1]]
            ...
            if (currentFin != lastFin[currentHand]) and (not prevMove[currentHand][0]):
                finDistance = (((abs(finHoriz - prevMove[currentHand][1]) ** 2) + (abs(finVert - prevMove[currentHand][2]) ** 2)) ** 0.5)
            else:
            ...
            patternBonus = rollNerf * movementBonus * rhythmBonus * holdBonus
        ...
        currentHand = math.floor(appliedFin / 2)
        currentFin = (appliedFin % 2)
        patternBonus = savedValues[appliedFin][0]
...
starRating = (sum(sectionStrains) ** 1.56) * 0.3
